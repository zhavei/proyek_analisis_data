...
Berikut merupakan tampilan sidebar yang akan dihasilkan dari kode tersebut. 
Nah, start_date dan end_date di atas akan digunakan untuk memfilter all_df. Data yang telah difilter ini selanjutnya 
akan disimpan dalam main_df. Proses ini dijalankan menggunakan kode berikut.'''

main_df = all_df[(all_df["order_date"] >= str(start_date)) & 
                (all_df["order_date"] <= str(end_date))]

'''DataFrame yang telah difilter (main_df) inilah yang digunakan untuk menghasilkan berbagai DataFrame yang dibutuhkan 
untuk membuat visualisasi data. Proses ini tentunya dilakukan dengan memanggil helper function yang telah kita buat sebelumnya.'''
...
#kita juga perlu menyertakan informasi tentang performa penjualan dari setiap produk. 
#Pada proyek ini, kita akan menampilkan 5 produk paling laris dan paling sedikit terjual melalui sebuah visualisasi data.
...
Pada materi Latihan Membuat Visualisasi Data, kita telah membuat tiga buah visualisasi data untuk menjelaskan hal tersebut. Nah, 
...
'''Informasi terakhir yang harus kita tampilkan ialah terkait parameter RFM 
(Recency, Frequency, & Monetary). Nah, pada proyek ini, kita akan menampilkan 
nilai average atau rata-rata dari ketiga parameter tersebut menggunakan widget metric(). 
